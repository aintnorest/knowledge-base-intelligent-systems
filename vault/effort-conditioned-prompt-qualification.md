...
First-party, GSL resume pipeline (`gsl` repo, `docs/resume-prompt-decisions.md`, entries V2z and V3f, plus the post-V3f production record):
...
- **Production confirmation of the effort-down fix.** The first production run after medium shipped on `resume_revise` finished in 530s against 55k output tokens, inside the extrapolated 300-900s envelope. The high-effort production distribution on the identical prompt and model was 571 / 1148 / 1221 seconds plus two runs killed at the 1800s deadline, at 118-128k output tokens. The tail is gone and the token volume fell by more than half, on an empty prompt diff — which is the page's claim stated as a measurement: the effort dial alone moved the system from fatally tailed to comfortable.

## Limitations

- The medium revision configuration was qualified at fixture scale only, and its production envelope was extrapolated from the 49-116s fixture range rather than measured — the standing caveat from the same incident being that fixtures are too small to trigger production-scale deliberation. **The first production run has since confirmed it** (530s, 55k output tokens, against 571-1221s and 118-128k at high with two deadline kills), so the mechanism argument is no longer carrying the claim alone. One production run is confirmation of direction and magnitude, not yet a distribution.
