...
The instruction-finetuning entry in the 2022 catalog is the clearest case of a threshold that did not hold. Within months, the Flan paper reported that instruction finetuning improved the held-out normalized average at *every* size it tested, from an 80M encoder-decoder to 540B, and that a 3B Flan-T5 beat GPT-3 175B on MMLU. The honest reading is intermediate rather than a refutation: the small finetuned models improved on the aggregate while still sitting near or below the random-guessing baseline in absolute terms, so whether the ability was "present" depends on where the bar is drawn — which is the page's metric caveat restated as a threshold dispute. A separate finding from the same work cuts the other way and supports the catalog: the benefit of CoT *prompting* remained scale-gated even after instruction finetuning, appearing on BBH only for the 540B models and one 62B variant.
...
- [Scaling Instruction-Finetuned Language Models dossier](/dossiers/scaling-instruction-finetuned-language-models.md) - months-later evidence that instruction-finetuning gains extend down to 80M models, qualifying the catalog's ~100B threshold, while CoT-prompting benefit stayed scale-gated even after finetuning.
